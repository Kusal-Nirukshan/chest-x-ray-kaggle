...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--guided-mode", choices=["residual", "multiply", "none"], default="multiply")
    parser.add_argument("--lambda-att", type=float, default=1.0)
    parser.add_argument("--lambda-bg", type=float, default=0.0)
...
def _flatten_efficiency(row: Dict[str, Any]) -> Dict[str, Any]:
    aliases = {
        "params": ("params", "parameters", "num_params"),
...
def main() -> None:
    ...
    output_dir = args.output_dir or args.repo_root / "artifacts" / "trustworthiness" / args.backbone
    ...
    write_master_comparison(master_rows, args.repo_root / "artifacts" / "trustworthiness" / "master_comparison.csv")
    write_validation_selection_table(validation_rows, output_dir / "validation_selection.csv")

    print("\nTrustworthiness comparison:")
    print(master.to_string(index=False))

    zip_base = output_dir.parent / f"{args.backbone}_trustworthiness_pair_results"
