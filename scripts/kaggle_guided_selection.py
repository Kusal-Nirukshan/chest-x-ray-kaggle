...
def main() -> None:
    ...
    train_loader, val_loader, _test_loader, class_names, train_targets, _datasets = build_dataloaders(
    ...
    for guided_mode in args.guided_modes:
        for lambda_att in args.lambda_att_values:
            for lambda_bg in args.lambda_bg_values:
                candidate_name = f"{guided_mode}_att{lambda_att}_bg{lambda_bg}".replace(".", "p")
